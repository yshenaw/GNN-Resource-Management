...
def build_graph(path_loss, dist, norm_dist, K, threshold):
    n = path_loss.shape[0]
    ...
    y = torch.tensor(np.expand_dims(path_loss,axis=0), dtype=torch.float)
    data = Data(x=x, edge_index=edge_index.contiguous(),edge_attr = edge_attr, y = y)
    return data

def proc_data(HH, dists, norm_dists,K):
    n = HH.shape[0]
    data_list = []
    for i in range(n):
        data = build_graph(HH[i,:,:],dists[i,:,:], norm_dists[i,:,:],K,300)
...
train_data_list = proc_data(train_path_losses, train_dists, norm_train, train_K)
test_data_list = proc_data(test_path_losses, test_dists, norm_test, test_K)
...
gen_tests = [20, 50, 100, 200, 500, 1000]
for test_K in gen_tests:
    ...
    crossLink_channel_losses = helper_functions.get_crossLink_channel_losses(test_path_losses)
    start = time.time()
    Y = FP_optimize(test_config,test_path_losses,np.ones([test_layouts, test_K]))
    end = time.time()
    ...
    bl_Y = simple_greedy(test_path_losses,np.ones([test_layouts, test_K]),Y)
    ...
    _, norm_test = normalize_data(1/train_dists,1/test_dists)
    test_data_list = proc_data(test_path_losses, test_dists, norm_test, test_K)
    test_loader = DataLoader(test_data_list, batch_size=test_layouts, shuffle=False, num_workers=1)
    loss2 = test()
    print('CGCNet:',loss2)
